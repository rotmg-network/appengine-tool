...
async function readJar(origin, url) {
  const jar = cookieJars.get(origin);
  if (!jar) return "";
  return jar.getCookieString(url);
}

async function listJar(origin, url) {
  const jar = cookieJars.get(origin);
  if (!jar) return [];
...
async function writeJar(origin, url, setCookies) {
...
function buildBody(bodyType, formParams = [], rawBody = "") {
...
app.delete("/api/cookies", (request, response) => {
  const key = String(request.query.environment || "production");
  const baseUrl = environments[key] || environments.production;
...
app.post("/api/send", async (request, response) => {
  ...
  const resolvedBaseUrl = payload.customBaseUrl || environments[payload.environment] || environments.production;
  ...
  const timeout = setTimeout(() => abortController.abort(new Error(`Request timed out after ${timeoutMs} ms`)), timeoutMs);
  const headers = {};
  ...
  try {
    ...
    response.status(timedOut ? 504 : 502).json({
      ok: false,
      error: timedOut ? `Request timed out after ${timeoutMs} ms` : error instanceof Error ? error.message : String(error),
      elapsedMs,
      timeoutMs,
      rawRequest
    });
